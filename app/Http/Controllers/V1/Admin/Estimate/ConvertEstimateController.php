...
class ConvertEstimateController extends Controller
{
    ...
    public function __invoke(Request $request, Estimate $estimate)
    {
        ...
        $invoiceId = Cache::lock("invoice-number:{$companyId}", 10)->block(5, function () use ($estimate, $request, $invoice_date, $due_date, $templateName) {
            $attempts = 0;
            $maxAttempts = 3;

            while ($attempts < $maxAttempts) {
                try {
                    return DB::transaction(function () use ($estimate, $request, $invoice_date, $due_date, $templateName) {
            ...
            throw new \RuntimeException('Failed to convert estimate after maximum attempts');
        });
